...
CREATE TABLE da_999.entity_sample_to_dq_rule_hist (
      id uuid NOT NULL,
      entity_sample_id uuid NULL,
      dq_rule_id uuid NOT NULL,
      settings text NULL,
...
CREATE OR REPLACE FUNCTION da_999.entity_sample_to_dq_rule_history()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
declare
...
begin
	if (TG_OP = 'UPDATE') then
select max(version_id) into max_version_id from da_999.entity_sample_to_dq_rule where id = new.id;
...
insert into da_999.entity_sample_to_dq_rule_hist
